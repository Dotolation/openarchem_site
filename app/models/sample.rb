class Sample < ActiveRecord::Base
	...
	def self.get_data(oa_id)
 		doc = Sample.find_sample(oa_id).first
 		...
 		site_data, source_data, extraction_data, equipment_data, chromatogram_data = Sample.get_data_for_inner_list(sam_hash["site_id"], sam_hash["source_id"], sam_hash["equipment_id"], sam_hash["extraction_id"], sam_hash["chromatogram_id"])
 		
 		return {"show_hash" => sam_hash, "inner_fields" => link_fields, "site_id" => site_data, "source_id" => source_data, "extraction_id" => extraction_data, "equipment_id" => equipment_data, "chromatogram_id" => chromatogram_data}
	end
	...
	def self.get_data_for_inner_list(site_id, source_id, equipment_id, extraction_id, chromatogram_id)
		...
		return site_data, source_data, extraction_data, equipment_data, chromatogram_data
	end
